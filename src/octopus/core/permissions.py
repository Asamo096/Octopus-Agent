...
class PermissionEngine:
    ...
    def __init__(self, settings: dict[str, Any] | None = None):
        ...
        self.dangerous_commands: set[str] = set(
            self.settings.get("dangerous_commands", [])
        )
        ...
        if self.mode == PermissionMode.PLAN:
            if self._is_write_operation(tool_call):
                return PermissionResult(
                    allowed=False, reason="Write operations blocked in plan mode"
                )
    ...
    def _check_file_operation(self, tool_call: Any, context: Any) -> PermissionResult:
        ...
        if not path:
            return PermissionResult(allowed=False, reason="No path specified")
        ...
        if self.mode == PermissionMode.DEFAULT:
            return PermissionResult(allowed=True, requires_approval=True)
    ...
    def _check_shell_operation(self, tool_call: Any, context: Any) -> PermissionResult:
        ...
        if not command:
            return PermissionResult(allowed=False, reason="No command specified")
    ...
    def _check_read_operation(self, tool_call: Any, context: Any) -> PermissionResult:
        ...
        if self._is_sensitive_path(expanded_path):
            return PermissionResult(
                allowed=False, reason=f"Access to sensitive path blocked: {path}"
            )
    ...
    def add_sensitive_path(self, pattern: str) -> None:
