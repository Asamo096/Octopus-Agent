    ...
    async def execute(self, data: Dict[str, Any]) -> HookResult:
        ...
        try:
            ...
            return HookResult(
                ...
                error=f"Hook {self.name} failed: {str(e)}"
            )
...
class HookManager:
    ...
    def unregister(self, event: HookEvent, hook_name: str):
...
def register_default_hooks(hook_manager: HookManager):
    ...
    hook_manager.register(
        HookEvent.AUDIT_LOG,
        Hook("audit_log", audit_log_hook, priority=50)
    )
